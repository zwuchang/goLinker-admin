    ...
    <el-form ref="form" :model="config" label-width="240px">
      ...
      <el-tabs v-model="activeNames">
        <el-tab-pane label="系统配置" name="1" class="mt-3.5">
          ...
          <el-form-item label="邀请链接基础URL">
            <el-input
              v-model.trim="config.system['invite-base-url']"
              placeholder="请输入邀请链接基础URL，如：https://your-domain.com/invite"
            />
          </el-form-item>
          <el-tooltip
...
<script setup>
  ...
  const config = ref({
    system: {
      ...
      'iplimit-time': 0,
      'router-prefix': '/api',
      'invite-base-url': 'https://your-domain.com/invite'
    },
    jwt: {
      'signing-key': '',
      'expires-time': '',
  ...
  const initForm = async () => {
    try {
      ...
      if (res.code === 0) {
        ...
        config.value = mergeConfig(config.value, serverConfig)
      }
    } catch (error) {
  ...
  // 监听配置变化，确保所有属性都存在
  const ensureConfigProperties = () => {
    try {
      // 确保所有必要的属性都存在
      if (!config.value.system) config.value.system = {}
      if (!config.value.jwt) config.value.jwt = {}
      if (!config.value.mysql) config.value.mysql = {}
      if (!config.value.redis) config.value.redis = {}
    } catch (error) {
      handleError(error)
    }
  }
  ...
  const update = async () => {
    try {
      ensureConfigProperties() // 确保配置完整性
  ...
  const getWeb3UUID = () => {
    config.value['web3-jwt']['signing-key'] = CreateUUID()
  }
