...
// GetSpec 获取任务规则
func (tm *taskManager) GetSpec(id cron.EntryID) string {
...
// GetTask 获取任务信息
func (tm *taskManager) GetTask(id cron.EntryID) *task {
	return tm.tasks[id]
}

// GetAllTasks 获取所有任务
func (tm *taskManager) GetAllTasks() map[cron.EntryID]*task {
	return tm.tasks
}

// GetEntries 获取定时任务条目
func (tm *taskManager) GetEntries() []cron.Entry {
	if tm.corn == nil {
		return nil
	}
	return tm.corn.Entries()
}

// GetDescription 获取任务描述
func (tm *taskManager) GetDescription(id cron.EntryID) string {
	if task, ok := tm.tasks[id]; ok {
